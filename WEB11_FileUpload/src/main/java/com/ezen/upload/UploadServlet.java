...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int uploadFileSizeLimit = 5 * 1024 * 1024; // 5MB로 제한
		...
		try {
			...
			// "multipart/form-data로 보낸 데이터들은 일반 request로 전달받은 수 없습니다."
			// 따라서 MultipartRequest에 request를 넣어서 복합사용되게 설정합니다.		
			uploadFilePath, // 서버상의 실제 디렉토리
			uploadFileSizeLimit, // 최대 업로드 파일 크기
			encType, // 인코딩 방법
			new DefaultFileRenamePolicy() // 업로드 파일과 동일 이름이 이미 존재하면 새이름 부여
			);
			...
			// 서블릿에서 웹브라우저에 직접 내용을 써넣기 위한 ContentType타입 설정
			...
			System.out.println("파일 업로드 되지 않았음 : " + e);
		}
	}
