...
public class MultiFileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 하나의 폼 안에 있던 <input type="file">의 파일들은 MultipartRequest 생성시 한 번에 업로드됩니다.
			
			// 다만 전달된 파라미터의 file은 multi.getFileNames();로 전달받고 ,
			// Enumeration으로 하나하나 꺼내서 사용합니다.
			Enumeration files = multi.getFileNames(); // 파일들의 객체를 추출
			while(files.hasMoreElements()) { // 파일의 요소의 객수 만큼 반복 실행
				String file = (String) files.nextElement(); // 각각의 파일 객체 추출
				String file_name = multi.getFilesystemName(file); // 업로드된 파일 이름 추출
				// DefaultRenamePolicy에 의해 구분된 실제 파일 이름 추출
